    ...
    ILendingAdapterWithRewards
{
    ...
    function _validateMarket(
        ...
        address underlying;
        try ICToken(market.marketAddress).underlying{gas: 50_000}() returns (
            address token
        ) {
            underlying = token;
        } catch {
        ...
        if (underlying != market.collateralToken)
            revert MarketValidationFailed(
                "loanToken does not match underlying of cToken"
            );
    ...
    function getSupplyCallData(
        ...
        target = market.marketAddress; // cToken contract
    ...
    function getSupplyBalance(
        ...
        address cToken = market.marketAddress;
        ...
        returns (address[] memory targets, bytes[] memory callDatas)
    {
        ...
        cTokens[0] = market.marketAddress;
        ...
        // 2. borrow
        targets[1] = market.marketAddress;
    ...
    function getClaimRewardsCallData(
        ...
        for (uint256 i = 0; i < markets.length; i++) {
            if (markets[i].marketAddress == address(0)) {
                revert CTokenNotFound(address(0));
            }
            cTokens[i] = ICToken(markets[i].marketAddress);
        }
    ...
    function getUserPosition(
        ...
        for (uint256 i = 0; i < markets.length; i++) {
            TezoroLendingLib.Market calldata market = markets[i];
