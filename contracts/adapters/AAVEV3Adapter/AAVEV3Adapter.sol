...
contract AAVEV3Adapter is ERC165, ILendingAdapter, ILendingAdapterWithRewards {
    ...
    function getClaimRewardsCallData(
        ...
        for (uint256 i = 0; i < markets.length; i++) {
            address aToken = markets[i].marketAddress;
            if (aToken != address(0)) {
    ...
    function getSupplyBalance(
        ...
        (uint256 aTokenBalance, , , , , , , , ) = IPoolDataProvider(
            dataProvider
        ).getUserReserveData(market.loanToken, user);
        return aTokenBalance;
    }

    function _validateMarket(TezoroLendingLib.Market memory key) internal view {
        (address aTokenAddress, , ) = IPoolDataProvider(dataProvider)
            .getReserveTokensAddresses(key.collateralToken);
        if (aTokenAddress == address(0))
            revert MarketValidationFailed(
                "aTokenAddress not found for collateralToken"
            );
        if (aTokenAddress != key.marketAddress)
            revert MarketValidationFailed(
                "marketAddress should be aTokenAddress"
            );

        if (key.auxId != bytes32(0))
    ...
    function getRewardBalances(
        ...
        rewards = new TezoroLendingLib.RewardBalance[](markets.length);

        for (uint256 i = 0; i < markets.length; i++) {
            address[] memory rewardTokens = IRewardsController(
                rewardsController
            ).getRewardsByAsset(markets[i].marketAddress);
            ...
            for (uint256 j = 0; j < rewardTokens.length; j++) {
                amounts[j] = IRewardsController(rewardsController)
                    .getUserRewards(
                        _asSingleton(markets[i].marketAddress),
                        user,
                        rewardTokens[j]
                    );
            }
