...
contract MorphoBlueAdapter is ERC165, ILendingAdapter, Ownable {
    ...
    function supportsInterface(
        ...
        return
            id == type(ILendingAdapter).interfaceId ||
    ...
    function _validateMarket(TezoroLendingLib.Market memory key) internal view {
        if (key.marketAddress != address(0)) {
            revert MarketValidationFailed(
                "marketAddress must be zero in Morpho"
            );
        }
        if (key.flags != 0) {
        ...
        if (mp.loanToken != key.loanToken) {
            revert MarketValidationFailed("loanToken mismatch");
        }

        // TODO: Uncomment later
        // if (mp.collateralToken != key.collateralToken) {
        //     revert MarketValidationFailed("collateralToken mismatch");
        // }
    }
